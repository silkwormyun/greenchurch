...
void loop() {
  ...
  if (temperature < temperature_limit) {
     if (debug_mode == true){Console.println("Sending alert");}
...
void sendTempAlert(String message) {
  ...
  if (debug_mode == true){ Console.println("Running SendAnEmail...");}
  ...
  if (returnCode == 0) {
    if (debug_mode == true){Console.println("Success! Email sent!");}
  } else {
    // a non-zero return code means there was an error
    // read and print the error message
    while (SendEmailChoreo.available()) {
      char c = SendEmailChoreo.read();
      if (debug_mode == true){Console.print(c);}
    }
  } 
...
void runAppendRow(int lightLevel, float temperature, float humidity) {
  ...
  if (returnCode == 0) {
    ...
    if (debug_mode == true){ Console.println(); }
  }
